# KingPi — Incremental Implementation Plan
...
- `src/kingpi/schemas/event.py` — Pydantic models: `EventType` enum (install/uninstall), `EventIn(timestamp: datetime, package: str, type: EventType)` with package name regex validation (`^[a-zA-Z0-9._-]+$`) and ISO 8601 timestamp, `EventSummary(count, last)`
- `src/kingpi/schemas/package.py` — `PackageResponse(info, releases, events)`
- `src/kingpi/schemas/error.py` — `ErrorResponse(detail: str, status_code: int)` — consistent error envelope for all error responses
- `src/kingpi/services/event_store.py` — **Abstract `EventStore` protocol** + `InMemoryEventStore` implementation
...
- `src/kingpi/services/pypi_client.py` — `fetch_package_info(package)` using `httpx.AsyncClient` to call `https://pypi.python.org/pypi/{package}/json`. Configurable timeout via `pypi_request_timeout_seconds` in config. Raises `PackageNotFoundError` on 404, distinct error on 5xx upstream failures.
- `src/kingpi/api/events.py` — `POST /event`
- `src/kingpi/api/packages.py` — `GET /package/{name}`, `GET /package/{name}/event/{type}/total`, `GET /package/{name}/event/{type}/last`
- `src/kingpi/api/health.py` — `GET /health` — basic health check endpoint
- `src/kingpi/app.py` — Wire routers, lifespan for httpx client, CORSMiddleware with configurable origins
- `src/kingpi/dependencies.py` — FastAPI dependency injection for store & client; `Settings` available via `Depends()` for testability
...
Files:
- `src/kingpi/services/cache.py` — `Cache` protocol + `InMemoryTTLCache` (dict + expiry timestamps)
- Modify `pypi_client.py` — Wrap fetches with cache lookup (cache-aside pattern)
- `src/kingpi/config.py` — Add `pypi_cache_ttl_seconds: int = 300`
...
Files:
- `src/kingpi/models/event.py` — SQLAlchemy model: `PackageEvent(id, package, event_type, count, last_timestamp)` — one row per (package, type) pair with atomic counter updates
- `src/kingpi/services/pg_event_store.py` — `PostgresEventStore` implementing `EventStore` protocol
...
- `src/kingpi/db/engine.py` — Async SQLAlchemy engine + session factory
...
Files:
- `src/kingpi/services/redis_cache.py` — Redis-backed cache implementing `Cache` protocol
...
3. Manual: `uvicorn src.kingpi.app:app --reload` then curl endpoints
